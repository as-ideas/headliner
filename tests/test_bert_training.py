...
class TestBertTraining(unittest.TestCase):
    ...
    def test_training_summarizer_bert(self):
        summarizer_bert = SummarizerBert(num_heads=4,
        ...
        summarizer_bert.optimizer_encoder = tf.keras.optimizers.Adam(learning_rate=3e-5)
        summarizer_bert.optimizer_decoder = tf.keras.optimizers.Adam(learning_rate=1e-4)
        ...
        output_bert = summarizer_bert.predict_vectors('I love dogs.', '')
        expected_first_logits = np.array([-2.069179, -1.594622,  1.158607,  3.03027, 1.404088])
        np.testing.assert_allclose(expected_first_logits, output_bert['logits'][0], atol=1e-6)
        self.assertEqual('[CLS] I love dogs. [SEP]', output_bert['preprocessed_text'][0])
        self.assertEqual('Dogs. [SEP]', output_bert['predicted_text'])
